...
int main(void) {
    ...
    // Ausgabe-Dateien
    FILE* pos_file = fopen("data/A16_Verlet_Integrator.csv", "w");
    FILE* energy_file = fopen("data/A16_Verlet_Integrator_Energie.csv", "w");
    ...
    while (t < T_max) {
        cvc_verlet_step(t, delta_t, y, pendumlumsODE, dimension, NULL);     // Aufruf der Integrationsfunktion
